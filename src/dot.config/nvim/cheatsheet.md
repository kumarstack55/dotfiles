...
ノーマルモードで `/<C-r>"` で `"` レジスタの中身をペーストできる。

### リテラルで検索する。

`/` は正規表現で検索する。
そのため、例えば `/` を含む文字列の検索はエスケープが必要となる。

リテラルで検索したい場合は、検索レジスタに検索文字列を入れることでできる。
以下に `"` レジスタの中身を検索する場合の例を示す。
...
次の箇所にカーソルがあるとき、 `gF` で指定パスの指定行に移動できる。

```
filename.txt:123
```

https://twitter.com/YKirin0418/status/1500094322561085441
